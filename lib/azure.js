...
const { StorageSharedKeyCredential, ContainerClient, generateBlobSASQueryParameters, SASProtocol } = require('@azure/storage-blob');
...
class ContainerAzure {
    ...
    constructor(auth, containerName, options) {
        ...
        this.sharedKeyCredentials = new StorageSharedKeyCredential(
            this.accountName,
            auth.accountKey
        );
    ...
    async validate() {
        ...
        if (result && result._response && result._response.status === 200) {
            return true;
        }
    }
    ...
    async listObjects(prefix) {
        ...
        const iter = this.containerClient.listBlobFlat(options);
        if(iter){
