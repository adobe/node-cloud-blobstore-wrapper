...
class ContainerAzure {
    ...
    constructor(auth, containerName, options) {
        ...
        this.sharedKeyCredentials = new storage.StorageSharedKeyCredential(
            this.accountName,
            auth.accountKey
        );
        ...
        this.containerClient = new storage.ContainerClient(
            url,
            storage.newPipeline(this.sharedKeyCredentials, {})
        );
    }
    ...
    async listObjects(prefix) {
        ...
        const response = await this.containerClient.listBlobFlat(options);
        for (const blobItem of response) {
            result.push({
                name: blobItem.name,
